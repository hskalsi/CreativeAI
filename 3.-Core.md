...
This function is very similar to [```generateSentence```](./3.-Core#generatesentence). The only difference is that it calls the [```getNextNote```](#getnextnote) function for its language models instead of the ```getNextToken``` function.
